...
microsoft_graph_clear_request_builder = lazy_import('msgraph.generated.drives.item.items.item.workbook.worksheets.item.charts.item.axes.series_axis.minor_gridlines.format.line.microsoft_graph_clear.microsoft_graph_clear_request_builder')
...
class LineRequestBuilder():
    ...
    async def get(self,request_configuration: Optional[LineRequestBuilderGetRequestConfiguration] = None) -> Optional[workbook_chart_line_format.WorkbookChartLineFormat]:
        ...
        if not self.request_adapter:
            raise Exception("Http core is null") 
        return await self.request_adapter.send_async(request_info, workbook_chart_line_format.WorkbookChartLineFormat, error_mapping)
