...
if TYPE_CHECKING:
    ...
    from .....models.cloud_clipboard_item_collection_response import CloudClipboardItemCollectionResponse
...
class ItemsRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, Dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/users/{user%2Did}/cloudClipboard/items{?%24count,%24expand,%24filter,%24orderby,%24search,%24select,%24skip,%24top}", path_parameters)
    
    def by_cloud_clipboard_item_id(self,cloud_clipboard_item_id: str) -> CloudClipboardItemItemRequestBuilder:
        """
        Provides operations to manage the items property of the microsoft.graph.cloudClipboardRoot entity.
        ...
        url_tpl_params["cloudClipboardItem%2Did"] = cloud_clipboard_item_id
        return CloudClipboardItemItemRequestBuilder(self.request_adapter, url_tpl_params)
    
    async def get(self,request_configuration: Optional[ItemsRequestBuilderGetRequestConfiguration] = None) -> Optional[CloudClipboardItemCollectionResponse]:
        """
    ...
    async def post(self,body: Optional[CloudClipboardItem] = None, request_configuration: Optional[ItemsRequestBuilderPostRequestConfiguration] = None) -> Optional[CloudClipboardItem]:
        """
    ...
    def to_post_request_information(self,body: Optional[CloudClipboardItem] = None, request_configuration: Optional[ItemsRequestBuilderPostRequestConfiguration] = None) -> RequestInformation:
        """
        Create new navigation property to items for users
        param body: The request body
        param request_configuration: Configuration for the request such as headers, query parameters, and middleware options.
        ...
        if not body:
            raise TypeError("body cannot be null.")
        request_info = RequestInformation(Method.POST, '{+baseurl}/users/{user%2Did}/cloudClipboard/items', self.path_parameters)
        request_info.configure(request_configuration)
        request_info.headers.try_add("Accept", "application/json")
