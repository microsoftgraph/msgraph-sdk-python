...
class ContentRequestBuilder(BaseRequestBuilder):
    ...
    def to_put_request_information(self,body: bytes, request_configuration: Optional[RequestConfiguration] = None) -> RequestInformation:
        ...
        if not body:
            raise TypeError("body cannot be null.")
        request_info = RequestInformation(Method.PUT, '{+baseurl}/users/{user%2Did}/contactFolders/{contactFolder%2Did}/childFolders/{contactFolder%2Did1}/contacts/{contact%2Did}/photo/$value', self.path_parameters)
        request_info.configure(request_configuration)
